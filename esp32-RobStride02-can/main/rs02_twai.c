...
esp_err_t rs02_twai_install(gpio_num_t tx_gpio, gpio_num_t rx_gpio)
{
    ...
    general.alerts_enabled = RS02_TWAI_ALERTS;

    rs02_twai_timing_spec_t timing_spec = {0};
    if (!rs02_twai_timing_1mbps_80_percent(&timing_spec)) {
        return ESP_ERR_INVALID_STATE;
    }
...
{
    if (!s_started || frame == NULL || frame->identifier > RS02_CAN_ID_MAX ||
...
esp_err_t rs02_twai_receive(rs02_frame_t *frame, TickType_t ticks_to_wait)
{
    ...
    if (!message.extd || message.rtr || message.data_length_code != RS02_CAN_PAYLOAD_SIZE ||
        message.identifier > RS02_CAN_ID_MAX) {
